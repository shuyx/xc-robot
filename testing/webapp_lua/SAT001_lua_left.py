...
# -*- coding: utf-8 -*-
"""
SAT001 Lua Left Arm Connection Test
基于webapp lua调用的左臂连接测试
...
class LuaLeftArmTest:
    """左臂Lua连接测试类"""
    
    def __init__(self, robot_ip: str = "192.168.58.3"):
        """
        初始化左臂lua测试
        ...
        self.test_name = "SAT001_LUA_LEFT"
        self.lua_script_name = "sat001_left_test.lua"
    ...
    def _create_lua_script(self) -> str:
        """创建左臂连接测试的lua脚本内容"""
        lua_script = """
-- SAT001 Left Arm Connection Test Script
-- 左臂连接测试lua脚本

function test_left_arm_connection()
    -- 初始化测试结果
    local test_result = {
    ...
    if error_code == 0 then
        ...
        TPWrite("Left arm SDK version: " .. version)
    else
    ...
    if state_error == 0 then
        ...
        TPWrite("Left arm robot state: " .. robot_state)
    else
    ...
    if joint_error == 0 then
        ...
        TPWrite("Left arm joint positions retrieved successfully")
        for i = 1, 6 do
    ...
    if test_result.connection_status then
        ...
        if move_error == 0 then
            TPWrite("Left arm basic movement test passed")
            Sleep(1000)  -- 等待1秒
            -- 回到原位
            RelMoveJTraj(0, {-1.0, 0, 0, 0, 0, 0}, 20.0, 20.0, 100.0, 0.0, 0.0, 1, 0)
        else
            test_result.error_code = move_error
            TPWrite("Left arm movement test failed, error: " .. move_error)
        end
    end
    
    TPWrite("Left arm connection test completed")
    return test_result
end

-- 执行测试
local result = test_left_arm_connection()
...
"""
    ...
    def connect_robot(self) -> bool:
        """连接到左臂机器人"""
        try:
            self.logger.info(f"正在连接左臂机器人: {self.robot_ip}")
            ...
            self.logger.error(f"连接左臂时发生异常: {str(e)}")
            return False
    ...
    def run_test(self) -> Dict[str, Any]:
        ...
        try:
            # 阶段1: 连接机器人
            self.logger.info("开始SAT001左臂Lua连接测试")
