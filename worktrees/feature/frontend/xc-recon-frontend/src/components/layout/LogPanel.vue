      ...
      <div class="action-bar">
        ...
        <div class="action-buttons">
          ...
          <div class="export-dropdown" ref="exportDropdownRef">
            <button @click="toggleExportDropdown" class="icon-button" title="导出日志">
              <el-icon>
                <Download />
              </el-icon>
            ...
            <div v-if="showExportDropdown" class="dropdown-menu">
              <div class="dropdown-item" @click="exportLogs('csv')">
                <el-icon><Document /></el-icon>
                导出为CSV
              </div>
              <div class="dropdown-item" @click="exportLogs('md')">
                <el-icon><Document /></el-icon>
                导出为Markdown
              </div>
              <div class="dropdown-item" @click="exportLogs('txt')">
                <el-icon><Document /></el-icon>
                导出为文本
              </div>
              <div class="dropdown-item" @click="exportLogs('json')">
                <el-icon><Document /></el-icon>
                导出为JSON
              </div>
            </div>
...
import { 
  ...
  WarningFilled, TrendCharts, ArrowDown 
...
const exportLogs = (format: 'csv' | 'md' | 'txt' | 'json') => {
  ...
  switch (format) {
    case 'csv':
      // CSV格式
      content = '时间,级别,消息\n' + 
        logsToExport.map(log => `"${log.time}","${log.level}","${log.message.replace(/"/g, '""')}"`).join('\n')
    ...
    case 'md':
      // Markdown格式
      content = `# 系统日志导出\n\n**导出时间**: ${new Date().toLocaleString()}\n**日志类型**: ${activeTab.value === 'errors' ? '错误日志' : '实时日志'}\n**日志数量**: ${logsToExport.length} 条\n\n---\n\n| 时间 | 级别 | 消息 |\n|------|------|------|\n` +
        logsToExport.map(log => `| ${log.time} | ${log.level} | ${log.message.replace(/\|/g, '\\|')} |`).join('\n') +
        `\n\n---\n\n*由 XC-OS v3.0 机器人系统生成*`
    ...
    case 'txt':
      // 文本格式
      content = `系统日志导出\n${'='.repeat(50)}\n导出时间: ${new Date().toLocaleString()}\n日志类型: ${activeTab.value === 'errors' ? '错误日志' : '实时日志'}\n日志数量: ${logsToExport.length} 条\n\n` +
        logsToExport.map(log => `[${log.time}] ${log.level.padEnd(5)} ${log.message}`).join('\n') +
        `\n\n${'='.repeat(50)}\n由 XC-OS v3.0 机器人系统生成`
    ...
    case 'json':
      ...
      const exportData = {
        meta: {
          ...
          exportType: activeTab.value === 'errors' ? 'error-logs' : 'realtime-logs',
  ...
  a.href = url
  a.download = `system-logs-${timestamp}.${fileExtension}`
...
// Setup periodic log simulation and event listeners
onMounted(() => {
