...
USER_ARM_POINTS = {
    "point1": {
        ...
        "position": [-54.14, -17.54, 36.47, -4.76, 147.66, -79.72],  # 关节角度 [J1,J2,J3,J4,J5,J6]
        "method": "MoveJ",
        "speed": 80.0,  # 自定义速度
    ...
    #当前TCP位置: X=-247.3, Y=-21.1, Z=129.1
    #当前TCP姿态: RX=179.8, RY=3.5, RZ=-10.8
    "point5": {
        "type": "joint",      # joint: 关节运动
        "position":  [-19.74, -61.3, 100.13, 47.7, 89.23, -98.97],  # 关节角度 [J1,J2,J3,J4,J5,J6]
        "method": "MoveJ",
        "speed": 80.0,  # 快速移动
        "wait_time": 0,  # 停留1秒
     ...
     # 你可以添加更多点位...
     "point2before": {
         #当前TCP位置: X=-443.7, Y=137.1, Z=-13.4
         #当前TCP姿态: RX=117.5, RY=79.9, RZ=-65.9
         "type": "joint",
         "position":  [-37.02, -34.02, 90.66, -51.04, 56.16, -84.15],
         "method": "MoveJ",
         "speed": 80.0,  # 快速移动 - 修正到合理范围
         "wait_time": 0,  # 短停留
         "description": "边缘位置(关节运动)"
    },
    ...
    "point2-2": {
        "type": "joint",      # joint: 关节运动
        "position":  [-29.35, -19.79, 55.05, -30.07, 63.8, -83.32],  # 关节角度 [J1,J2,J3,J4,J5,J6]
        "method": "MoveJ",
        "speed": 80.0,  # 中速移动 - 修正到合理范围
        "wait_time": 0,  # 很短停留
        "description": "边缘位置(关节运动)"
    },

    # 示例点位: 使用关节运动 (MoveJ) - 中速
    #当前TCP位置: X=-542.0, Y=137.1, Z=-13.4
    #当前TCP姿态: RX=117.5, RY=79.9, RZ=-65.9
    "point2": {
        "type": "joint",      # joint: 关节运动
        "position":  [-29.35, -19.79, 55.05, -30.07, 63.8, -83.32],  # 关节角度 [J1,J2,J3,J4,J5,J6]
        "method": "MoveJ",
        "speed": 20.0,  # 中速移动 - 修正到合理范围
        "wait_time": 0,  # 很短停留
        "description": "边缘位置(关节运动)"
    },
 # 示例点位: 使用关节运动 (MoveJ) - 不设置停留时间(使用默认)
    "point2high": {
        #当前TCP位置: X=-542.0, Y=81.3, Z=-13.4
        #当前TCP姿态: RX=117.5, RY=79.9, RZ=-65.9
        #关节角度[23.98, -3.98, 18.84, -10.07, 116.95, 11.67]
        "type": "joint",      # joint: 关节运动
        "position": [-22.87, -22.4, 61.19, -33.85, 70.25, -82.7],  # 关节角度 [J1,J2,J3,J4,J5,J6]
        "method": "MoveJ",
        "speed": 20.0,  # 中等速度
        "wait_time": 0,  # 不设置wait_time则使用默认停留时间
        "description": "边缘位置(关节运动)"
    },
...
USER_MOTION_SEQUENCE = [
    "arm:point1",           # 机械臂到point1
    "chassis:1",            # 底盘导航到POI "1"
    "arm:point5",          # 机械臂到point11
    "arm:point2before",         
    "arm:point2-2",          # 机械臂到point22
    {"wait": 5000},
    "arm:point2high",
    "arm:point5",
    "chassis:3",            # 底盘导航到POI "3"
    "arm:point2high-2",          # 机械臂到point44
    "arm:point2",
    {"wait": 5000},         # 等待3秒
    "arm:point2high",
    "arm:point5",
    "chassis:home1",        # 底盘回home1
    "arm:point1"
]
