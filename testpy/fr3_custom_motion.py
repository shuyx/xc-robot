...
# 定义运动序列 (按你想要的顺序)
USER_MOTION_SEQUENCE = ["point11", "point55", "point66", "point77", "point66", "point77", "point66", "point33", "point44", "point33", "point44","point11"] 

# 你可以自定义运动序列，例如：
# USER_MOTION_SEQUENCE = ["home", "point1", "point2", "point3", "home"]
# USER_MOTION_SEQUENCE = ["point2", "point1"]  # 反向运动
# USER_MOTION_SEQUENCE = ["point1", "point2", "point1"]  # 往返运动
...
class FR3CustomController:
    ...
    def _validate_config(self):
        ...
        # 检查序列中的点位是否都存在
        missing_points = []
        for point_name in self.motion_sequence:
    ...
    def display_config(self):
        ...
        print(f"\n🚀 运动序列: {' → '.join(self.motion_sequence)}")
        print(f"📊 总计 {len(self.target_points)} 个点位，{len(self.motion_sequence)} 步运动")
        print("=" * 60)
    ...
    def execute_custom_motion(self):
        """执行自定义运动序列"""
        print("🎬 开始执行自定义运动序列")
        print(f"📋 运动路径: {' → '.join(self.motion_sequence)}")
        ...
        for i, point_name in enumerate(self.motion_sequence, 1):
            print(f"\n{'='*60}")
