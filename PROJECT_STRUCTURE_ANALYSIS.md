...
```
requirements.txt (主配置)
├── requirements/base.txt          # 核心依赖 (PyQt5, NumPy, OpenCV)
├── requirements/dev.txt           # 开发工具 (测试、调试)
├── requirements/monitoring.txt    # 监控图表 (matplotlib)
└── requirements/simulation_vtk.txt # VTK 3D仿真 (可选，380M)
```
...
| 指标 | 数量 | 说明 |
|------|------|------|
| **总文件大小** | 662M | 优化后精简容量，包含文档、代码、资源文件 |
| **Python代码文件** | 2,371个 | 核心业务逻辑和测试代码 |
| **Markdown文档** | 65个 | 完整文档体系和技术资料 |
| **测试用例** | 50+ | SAT/DAT系列专业测试 |
| **硬件支持** | 3大类 | FR3机械臂、Hermes底盘、Gemini相机 |
| **平台支持** | 3平台 | Windows、macOS、Linux |
| **容量优化** | 节省1.04GB | 从1.7GB→662M，61%空间节省 |
...
| **依赖管理** | ✅ 优化 | 95% | 分层结构，VTK可选，节省380M |
| **项目容量** | ✅ 优化 | 100% | 移除无效文件，精简项目结构 |
...
**容量优化** (project/):
- **项目大小**: 从1.7GB → 662M (减少61%)
- **依赖管理**: 分层结构，VTK可选依赖
- **清理效果**: 移除无效worktrees、Python缓存、冗余依赖
- **维护性**: 建立定期清理机制和容量监控
...
| 项目 | 优化前 | 优化后 | 节省空间 |
|------|---------|---------|----------|
| **项目总容量** | 1.7GB | 662M | 1.04GB (61%) |
| **无效worktrees** | 691M | 0M | 691M ✅ |
| **VTK依赖** | 380M | 0M | 380M ✅ |
| **Python缓存** | ~5M | 0M | 5M ✅ |
| **临时文件** | ~3M | 0M | 3M ✅ |
...
- ✅ **分层依赖管理**: requirements/目录结构
- ✅ **VTK可选化**: 移至simulation_vtk.txt，按需安装
- ✅ **智能提示**: 代码中增加Web端方案建议
- ✅ **向后兼容**: 现有代码仍可正常运行
- ✅ **维护性**: 建立定期清理机制
